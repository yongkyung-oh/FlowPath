...
#
assert dataset == 'PAM', 'Code for PAM'
...
for missing_ratio in missing_ratios:
    ...
    for k in range(n_splits):
        ...
        for m in range(n_runs):
            ...
            # save in numpy file
            if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':
                np.save('results_PAM/{}/'.format(dataset) + arch + '_{}.npy'.format(k), [args, missing_ratio, k, m, acc * 100, aupr * 100, auc * 100])
            elif dataset == 'PAM':
                np.save('results_PAM/{}/'.format(dataset) + arch + '_{}.npy'.format(k), [args, missing_ratio, k, m, acc * 100, aupr * 100, auc * 100, precision * 100, recall * 100, F1 * 100])
    ...
    # save in numpy file
    if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':
        np.save('results_PAM/{}/'.format(dataset) + arch + '.npy', [args, missing_ratio, acc_vec, auprc_vec, auroc_vec])
    elif dataset == 'PAM':
        np.save('results_PAM/{}/'.format(dataset) + arch + '.npy', [args, missing_ratio, acc_vec, auprc_vec, auroc_vec, precision_vec, recall_vec, F1_vec])
